<?php
    ...
    if(isset($_POST["InputNombre"]) && isset($_POST["idvalue"]) && isset($_POST["InputPassword"]))
    {
        ...
        $stmt = $conn->prepare("UPDATE admins SET admin_name=?, passwords=? WHERE admins_id=?");
    ...
    elseif(isset($_POST["id"]))
    {
        ...
        $stmt = $conn->prepare("DELETE FROM admins WHERE admins_id=?");
        $stmt->bindParam(1, $id);
    ...
    elseif(isset($_POST["exampleInputNombre"]) && isset($_POST["exampleInputPassword"]))
    {
        ...
        $stmt = $conn->prepare("INSERT INTO admins(admin_name,passwords) VALUES(?,?)");
        $stmt->bindParam(1, $nombre);
        $stmt->bindParam(2, $passwords);
    ...
    //MOSTRAR TODO
        ...
        $data = $conn->query("SELECT admins_id, admin_name, passwords FROM admins")->fetchAll(PDO::FETCH_ASSOC);
